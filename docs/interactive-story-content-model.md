...
**Status: Stage 1 schema/prompt/validation and the catalogue write path (`write_interactive_draft`) are implemented and validated live; the illustration-to-page wiring gap (`write_illustrations`) is fixed generically but Stage 2 itself still doesn't understand this content model's shape.** This document is the living source of truth for this design — update it as the design evolves, don't let it drift out of sync with chat history.
...
- `write_interactive_draft` action added — `write_draft`'s counterpart, writing `entityChoices` + `branchPoints` metadata onto the `story_templates_table` item and every spine/branch page as its own row in `story_template_pages_table` (same key shape as today: `storyTemplateId` + `pageOrder`; a branch page's disambiguated order like `0004a` sorts correctly with no special-casing). Template rows are tagged `contentModel: "INTERACTIVE_V1"` so any future reader can tell the two shapes apart once both exist in the same table. Live-verified: wrote a real generated story to real DynamoDB, confirmed by reading the rows back directly (15 page rows, correct `entityChoices`/`branchPoints` nesting).
- **The `story_template_pages.baseLayers` gap called out below is now fixed** — a new `write_illustrations` action takes a reviewed Stage 2 plan whose assets have already been generated (`generate_illustrations` run and its jobs polled to completion), looks each one up by its deterministic asset id, and writes the resulting layers onto the right page's `baseLayers` (grouped by `forPageOrders`; the protagonist's constant `cast_base` master stays excluded, still resolved live). This also fixed the real rendering bug it was blocking: `_cast_base_layers` (story_instances) and `_resolve_layers` (story_preview) were unconditionally appending the protagonist's generic resting pose on every page even when a page-specific `cast_expression` variant existed — both now skip that default when a `cast_expression` override is present. Covered by new unit tests in `test_story_production.py` (happy path, rejects an asset that hasn't finished generating, rejects an unknown `pageOrder`). This work applies to the existing flat pipeline as much as the interactive one — it has no dependency on `entityChoices`/`branchPoints` and doesn't touch either.
- **Found and fixed while manually validating the above**: `reviewStatus` (the gate `_cast_base_layers`/slot-option reads filter on) was a leftover from the older hand-curated asset model (`backend/scripts/seed_story_content.py` is the only other place that ever wrote it) — the generative pipeline (`_ensure_generated_asset`) never set it at all, so every AI-generated asset was permanently invisible to those read paths regardless of the `write_illustrations` fix above. Resolved by auto-publishing: `_ensure_generated_asset` now sets `reviewStatus: "published"` the moment an asset is generated. No separate human-approval step exists or is planned for this pipeline — correction happens by tweaking the prompt/character bible and regenerating with `force_regenerate` (a new revision under the same identity), not by un-publishing.
...
Debugging the current (non-branching) illustration pipeline surfaced a foundational gap that this design inherits and must account for: **`story_template_pages.baseLayers` is written once, hardcoded to `[]`, at `write_draft` time, and nothing in the codebase ever populates it from generated illustration assets.** Even the protagonist currently shows one static pose on every page (`_cast_base_layers` only pulls the `cast_base` master, never the per-page `cast_expression` variants). This needed fixing regardless of the interactive-content-model work, and the two efforts are sequenced: background + protagonist wiring first (no dependency on this new content model), then Entity Choice illustration (which depends on this design's shape).

**Update: the background/protagonist wiring above is done** — see the `write_illustrations` entry in Implementation status. Entity Choice illustration (multi-option-per-choice art generation for `entityChoices`) is the remaining, still-unstarted half of this section — it's genuinely dependent on the interactive content model's shape (which option was picked determines which character/object art a page needs) and is the next real next-step, not this generic wiring fix.
...
Stage 1 (schema + prompt + validation), the catalogue write path (`write_interactive_draft`), and the generic illustration-to-page wiring (`write_illustrations`, plus the `cast_expression`-override fix in `story_instances`/`story_preview`) are implemented and validated (see Implementation status above; `write_illustrations` is unit-tested, not yet live-tested against Bedrock the way Stage 1 was). Not yet wired into: Stage 2's *input composition* (it doesn't yet understand `entityChoices`/`branchPoints` at all, and needs the multi-option-per-choice illustration extension — per-option character/object art — discussed earlier), or `story_instances` (needs the resolution step that flattens a template + one pick per choice into a linear, recordable page sequence — see Voice recording section above). Next step: pick one of those two and extend it.
