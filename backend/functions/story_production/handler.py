...
# The house-style reference itself is seeded with a genuinely blank image
# (see _ensure_house_style_reference), so it needs very little fidelity to
# that seed -- just enough to inherit "flat empty background", not so much
# that it can't add a subject at all. Confirmed live: 0.05-0.1 reliably
# produces an isolated subject on a plain background; stock text-to-image
# with no seed reliably did not (see prompt_compiler.compile_house_style_reference_prompt).
HOUSE_STYLE_REFERENCE_FIDELITY = 0.08
...
def lambda_handler(event, context):
    ...
    Actions:
      ...
      write_illustrations Populate story_template_pages.baseLayers from a
                  ...
                  to completion).
      generate_one_illustration Generate exactly one asset from a reviewed
...
def _generate_one_illustration(event):
    """Generate exactly one asset from a reviewed Stage 2 plan, synchronously,
    in this one call -- no job fan-out. Lets a producer validate a single
    prompt (and see it actually render) before spending Bedrock calls on the
    rest of the plan, and sidesteps generate_illustrations' async fan-out
    entirely, which is what the "Too many connections" Bedrock throttling
    under a large plan was actually coming from -- one asset at a time from
    the client is inherently serialized, no batching tuneable needed.

    A VARIANT_OF_IDENTITY here requires its NEW_IDENTITY's master to already
    exist (generated by an earlier call, individually or via
    generate_illustrations) -- same requirement generate_illustrations
    itself enforces for a batch.
    """
    theme_pack_id = _required_string(event, "themePackId")
    style_id = _required_string(event, "styleId")
    if style_id not in HOUSE_STYLES:
    ...
    if kind == "NEW_IDENTITY":
        scope_type = "CAST_MEMBER" if asset["kind"] == "PROTAGONIST" else "STORY"
        scope_id = cast_member_id if scope_type == "CAST_MEMBER" else story_template_id
        result = _ensure_master_character(
            scope_type, scope_id, asset["identityKey"], style_id, asset["layerType"],
            asset["characterBible"], asset["masterPrompt"],
            house_style_ref_b64, house_style_ref_fingerprint, force_regenerate,
        )
    elif kind == "VARIANT_OF_IDENTITY":
        ...
        variant_key = asset.get("variantKey")
        if not variant_key:
            raise InputError("asset", "asset.variantKey is required.")
        scope_type = "CAST_MEMBER" if identity_key == protagonist_identity_key else "STORY"
        scope_id = cast_member_id if scope_type == "CAST_MEMBER" else story_template_id
        master = _get_asset(_deterministic_asset_id(scope_type, scope_id, identity_key, style_id, "master"))
        if not master:
            raise InputError(
                "asset",
                f"No master asset exists yet for identity '{identity_key}'; "
                "generate its NEW_IDENTITY entry first.",
            )
        result = _ensure_character_variant(
            scope_type, scope_id, identity_key, variant_key, style_id, asset["layerType"],
            master, asset["mutation"], force_regenerate,
        )
    elif kind == "BACKGROUND":
        result = _ensure_background(
            "STORY", story_template_id, asset["identityKey"], style_id, asset["scene"],
            house_style_ref_b64, house_style_ref_fingerprint, force_regenerate,
        )
    elif kind == "PROCEDURAL_EFFECT":
        ...
        raise InputError("asset.assetKind", f"generate_one_illustration does not handle '{kind}'.")
...
def _ensure_house_style_reference(theme_pack_id, style_id):
    ...
    generate_blank_reference_seed) at low fidelity -- never plain
    text-to-image. Confirmed live: asking a pure text-to-image call for
    "one isolated woodland creature, plain background" reliably came back
    as a fully illustrated scene (a farmhouse, a human figure) instead;
    seeding Style Guide with empty content and low fidelity reliably
    produced an actually isolated subject on a plain background.
    ...
    started = time.monotonic()
    image_b64 = _invoke_style_guide(
        reference_prompt, blank_seed_b64, fidelity=HOUSE_STYLE_REFERENCE_FIDELITY, negative=negative,
    )
